...
Bayesian framework $$p(\theta|\mathcal{D}) = \frac{p(\mathcal{D}|\theta)p(\theta)}{p(\mathcal{D})}$$
...
of parameters as a random quantity, $\theta \sim p(\theta)$, where some values/realizations of $\theta$ are more
probable than others. In Bayesian regression we are interest is the predictive posterior distribution,  
\begin{align}\label{Predictive2}
    p(y|x, \mathcal{D}) &= \int p(y,\theta|x, \mathcal{D}) d\theta\\
    &= \int p(y|x,\theta)p(\theta|\mathcal{D}) d\theta,
\end{align}
where the posterior $p(\theta|\mathcal{D})$ gives weighting to the proposed regression model $p(y|x,\theta)$. 
Note that the second equation is true because of the probability chain rule and that $y$ is fully
described by the parametric model $p(y|x,\theta)$ and the parameters $\theta$ are fully described by
...
different versions of the MCMC method, Hamiltonian Monte Carlo - giving different results. As
revealed later (see in section \ref{BNN}), approximate inference might be flawed and inexact. 
...
point is chosen according to a so-called acquisition function, which balances out the well-known
concept of exploitation and exploration. It is exploitation if the next chosen location, $x \in
\mathcal{X}$ according to its average improvement. It is exploration if the next point is chosen in
a region of high uncertainty and thereby helps lower the overall uncertainty. First, we will look at
the acquisition function used in the thesis: Expected improvement. But also shortly go over
...
\begin{figure}[H]
    ...
    \caption{Contourplot of expected improvement for different (Gaussian) predictive
    uncertainties $\sigma_x = \sqrt{\mathbb{V}ar_{p(y|x,\mathcal{D})}[y]}$ versus the average
    improvement $y_{\min}-\mu_x$, where $\mu_x = E_{p(y|x,\mathcal{D})}[y]$. The blue line 
    is the acquisition function at Figure \ref{BO_example} for $x = [-100, 100]$ via the
    follwoing map $x \mapsto (\sigma_x, y_{\min}-\mu_x)$. The orange dot represent $x_{next} = 26.06$}
