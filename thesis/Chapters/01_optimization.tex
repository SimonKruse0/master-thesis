...
is key to its performance (statement in thesis \cite{} ok?). Explained here:
\begin{enumerate}
    ...
    function.
\end{enumerate}
Essentially, the only explorative phase of the deterministic surrogate optimization is the first initial samples
from the objective, where if done poorly, the optimization might get stuck in a local minima. This a consequence
of not represent uncertainties about the objective function. They dont pay attention to less-explored regaions 
very essential characteristics of a global search method. 

Bayesian formalism. Incorporate uncertainty into modelling. Our prior knowledge. about the characteristics. 
-> defining a prior distribtuion of possible functions.. 
1) Cope with noise
2) provide a uncertainty measure. And theoretical motivated aq functions ballance exploitation/exploration. 
3) Termination process -> when no more uncertainty?. 
