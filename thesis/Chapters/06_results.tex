...
\begin{table}[H]
  ...
  \begin{tabular}[t]{l}
    ...
    $p(\sigma) \sim \text{LogNormal}(0,1)$ \\
    $p(\sigma_{w}) \sim \text{InvGamma}$
  \end{tabular} &
  \begin{tabular}[t]{l}
...
\begin{testexample}[Cross-validation of hyperparamters]
...
\begin{table}[H]
  \centering
  \resizebox{\textwidth}{!}{\begin{tabular}[t]{p{.20\textwidth} | p{.40\textwidth}  | p{.40\textwidth}}
  \textbf{Model}       & \textbf{Specification}    &   \textbf{Hyperparamters} \\ \hline
  GMR& 
  \begin{tabular}[t]{l}
    Training sklearn GMM, \\
    MLE from EM with 20 restarts.\\
    Model inputed into the GMR\\ 
    framework \cite{GMR}. 
  \end{tabular} &
  \begin{tabular}[t]{l}
  ...
  \end{tabular} &
  \begin{tabular}[t]{l}
     Mixture components x-variance\\
     Mixture components y-variance\\
     Prior weight 
  \end{tabular} \\\hline

  SPN& 
  \begin{tabular}[t]{l}
    RAT-SPN (20 channels, 2 tracks)\\
    MAP found with 5 restarts of EM\\  
  \end{tabular} &
  \begin{tabular}[t]{l}
    Mixture components x-variance prior $\alpha_{x-dim}$ \\
    Mixture components y-variance prior $\alpha_{y-dim}$ \\
    Prior weight
  \end{tabular} \\\hline
...
with 24 different problems for continuous black-box optimization algorithms.
 ...
 \subsection{Uncertainty quantification}\label{UQ_score}
...
\ref{Test1_reg_visual}, showing one of the 10 regression results from fitting 36 data points. The
...
%This is seen in appendix \todo{Appendix}. 
...
of the points to the right, as a compromise, it keeps the large lengthscale and increases
...
As mentioned in Section \ref{BO_implementation}, for the follwoing experiments, the Bayesian
...
\begin{figure}[H]
  ...
  \begin{minipage}[b]{0.49\textwidth}
   ...
   \begin{minipage}[b]{0.49\textwidth}
    ...
    \end{minipage}
  \caption{Bayesian optimization of the four 1D Test functions average accross 20 different samples
  of initial dataset of size 5. BNN and GP win in Test1. SPN wins in test2. Random search wins in Test3. BNN wins in
  test4.}
  \label{Test_bayesOpt}
\end{figure}
...
In BO on Test1 (seen in Figure \ref{Test_bayesOpt} top left) it becomes evident that BOHamiANN does
not work, and GMR is as good as Random search. GP is a clear winner of Test1 as it reaches the
minimizer first. Test2 (Figure \ref{Test_bayesOpt} top right) shows after 35 iterations that random
search is better than the GP and that all the mixture models and (barely) the BNN performs better
than the GP. Finally, Test4 (Figure \ref{Test_bayesOpt} bottom right) shows approximately the same
results for KDE, SPN, BNN and GP, all being better than Random search and BOHamiANN. 

\section{Test on BBOB in higher dimensions}
So far, we have looked at self-defined 1D problems and can conclude that our regression models work
and that there exist problems in 1D where the GP is not the preferred model - in fact for
...
\cite{hansen2009real}, which consists of 24 functions divided into 5 different main classes and are
scaleable in dimensions. These functions are available through Comparing Continuous Optimizers
framework COCO \cite{COCO}, which is a popular benchmarking testbed for continuous black-box
optimization algorithms. We select the functions: f3, f9, f15 and f21 (from 3 every 6'th) which are
...
problems in three dimensions (2,3,5). Figure \ref{BBOB_regression} shows the regression performance for 3
representative problems, all of the $2\cdot 12$ plots can be seen in Appendix
\ref{BBOB_regression_all}.  

Noticeable, BNN suddenly gives relatively high uncertainty quantifications (better than empirical std)
and, unfortunately, high mean prediction error across all BBOB tests(besides f3 dim 2). This is
strange, and something might be wrong with the model. In contrast, BOHamiANN now has decent
predictive power but still has bad uncertainty quantifications (compared to the 1D tests). Besides
this surprise, the predictive power results are generally similar to the 1D test problems: The GP
has the best predictive power. The uncertainty quantification looks similar: Generative models are
better. 
...
It is hard to make a clear analysis of the BO results shown in Figures \ref{BBOB_bayesOpt},
\ref{BBOB_bayesOpt1}, \ref{BBOB_bayesOpt2}, \ref{BBOB_bayesOpt3}. In large dimensions, regression
models need more data for good performance due to the curse of dimensionality. This statement is
supported by the relatively good performance of the random search. Nevertheless, the GP is generally
the preferred model. BOHamiANN is surprisingly good, and BNN is doing worse than random search on
...
\begin{figure}[H]
  \centering
  \begin{minipage}[b]{0.32\textwidth}
   \includegraphics[trim=1cm 0.5cm 0.9cm 0.6cm,clip,width=\textwidth]{Figures/results_baysopt/3_2.pdf}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.32\textwidth}
    \includegraphics[trim=1cm 0.5cm 0.9cm 0.6cm,clip,width=\textwidth]{Figures/results_baysopt/3_3.pdf}
   \end{minipage}
   \hfill
   \begin{minipage}[b]{0.32\textwidth}
    \includegraphics[trim=1cm 0.5cm 0.9cm 0.6cm,clip,width=\textwidth]{Figures/results_baysopt/3_5.pdf}
   \end{minipage}
...
\begin{figure}[H]
  \centering
  \begin{minipage}[b]{0.32\textwidth}
   \includegraphics[trim=1cm 0.5cm 0.9cm 0.6cm,clip,width=\textwidth]{Figures/results_baysopt/9_2.pdf}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.32\textwidth}
    \includegraphics[trim=1cm 0.5cm 0.9cm 0.6cm,clip,width=\textwidth]{Figures/results_baysopt/9_3.pdf}
   \end{minipage}
   \hfill
   \begin{minipage}[b]{0.32\textwidth}
    \includegraphics[trim=1cm 0.5cm 0.9cm 0.6cm,clip,width=\textwidth]{Figures/results_baysopt/9_5.pdf}
   \end{minipage}
...
\begin{figure}[H]
  \centering
  \begin{minipage}[b]{0.32\textwidth}
   \includegraphics[trim=1cm 0.5cm 0.9cm 0.6cm,clip,width=\textwidth]{Figures/results_baysopt/15_2.pdf}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.32\textwidth}
    \includegraphics[trim=1cm 0.5cm 0.9cm 0.6cm,clip,width=\textwidth]{Figures/results_baysopt/15_3.pdf}
   \end{minipage}
   \hfill
   \begin{minipage}[b]{0.32\textwidth}
    \includegraphics[trim=1cm 0.5cm 0.9cm 0.6cm,clip,width=\textwidth]{Figures/results_baysopt/15_5.pdf}
   \end{minipage}
...
\begin{figure}[H]
  \centering
  \begin{minipage}[b]{0.32\textwidth}
   \includegraphics[trim=1cm 0.5cm 0.9cm 0.6cm,clip,width=\textwidth]{Figures/results_baysopt/21_2.pdf}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.32\textwidth}
    \includegraphics[trim=1cm 0.5cm 0.9cm 0.6cm,clip,width=\textwidth]{Figures/results_baysopt/21_3.pdf}
   \end{minipage}
   \hfill
   \begin{minipage}[b]{0.32\textwidth}
    \includegraphics[trim=1cm 0.5cm 0.9cm 0.6cm,clip,width=\textwidth]{Figures/results_baysopt/21_5.pdf}
   \end{minipage}
