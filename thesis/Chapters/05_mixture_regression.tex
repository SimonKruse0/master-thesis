...
\section{Conditional of mixture regression}
...
$$p(y|x) = p_z(y|x)  \hspace*{1cm} z \sim Cat(\gamma_1(x), \dots, \gamma_Z(x)) $$
And we name $p(z|x) = \gamma_z(x) \in [0,1]$ the \textit{responsibility} of mixture component $z$ 
at a given location $x \in \mathcal{X}$, (The probability that $y$ to belong to component $z$ at a given 
location $x$). 
...
The kernel density estimator is just a gaussian mixture models, with no correlation 
between any of the variables, i.e. $p_z(y|x) = p_z(y)$. More concretely we get
$$p(y|x) = \sum_{z=1}^n \gamma_z(x) \mathcal{N}(y|\mu_{y}^{(z)},\Sigma_{yy}^{(z)} )$$ with
$\gamma_z(x) := \frac{\lambda_z \mathcal{N}(x|\mu_{x}^{(z)},\Sigma_{xx}^{(z)})}{\sum_{z^*}
\lambda_{z^*} \mathcal{N}(x|\mu_{x}^{(z^*)},\Sigma_{xx}^{(z^*)})}$ where $z = 1,2,\dots, n$, which
is why the computational complexity is $O(n)$. Note that the denominator in $\gamma_z(x)$ can be
reused for all combonents. 

\subsection{Gaussian mixture model}
The Gaussian mixture regression is defined straight forward, 
$$p(y|x) = \sum_{z=1}^K \gamma_z(x) \mathcal{N}(y|\mu_{y|x}^{(z)},\Sigma_{y|x}^{(z)} )$$ 
with $\gamma_z(x) :=
\frac{\lambda_z \mathcal{N}(x|\mu_{x}^{(z)},\Sigma_{xx}^{(z)})}{\sum_{z^*} \lambda_{z^*}
\mathcal{N}(x|\mu_{x}^{(z^*)},\Sigma_{xx}^{(z^*)})}$
note here the computational complexity is only $O(K)$, 
...
\subsection{calculation of responsibility}
