% !TEX TS-program = xelatex

\documentclass[a4paper,twoside,11pt]{report} %openright
\input{Setup/Statics.tex}
\input{Setup/Preamble.tex}
\input{Setup/Settings.tex}
\usepackage{Setup/MoreSettings}
%\documentclass[tikz, border=1mm]{standalone}
% Allows us to include graphics
\usepackage{graphicx}
\usetikzlibrary{bayesnet}

\begin{document}
% %\subfile{Text/til_mikkel.tex}
%\input{Chapters/Text/04b_EM_for_mixtures.tex}

\pagenumbering{roman}
\input{Frontmatter/Frontpage.tex}
\pagecolor{white}
\newgeometry{top=2.81cm, bottom=2.75cm, outer=2.5cm, inner=3.5cm}
\pagestyle{empty}
\cleardoublepage 
\input{Frontmatter/Copyright.tex}
\clearpage 
\pagestyle{main}
% \input{Frontmatter/Approval.tex}
% \clearpage 
% \input{Frontmatter/Abstract.tex}
% \clearpage 

\input{Frontmatter/Acknowledgements.tex}
\cleardoublepage 
\tableofcontents
\cleardoublepage 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\pagenumbering{arabic}
\input{Chapters/Text/00_introduction.tex}

\cleardoublepage

\cleardoublepage
\input{Chapters/Text/01_optimization.tex}
\cleardoublepage

\input{Chapters/Text/02_bayesian_optimization.tex}
\cleardoublepage

\input{Chapters/Text/03_surrogate_models.tex}
\cleardoublepage

\input{Chapters/Text/04_inference.tex}
\cleardoublepage

\input{Chapters/Text/05_results.tex}

% % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\printbibliography[heading=bibintoc,title={Bibliography}]
\cleardoublepage 
\appendix
\input{Backmatter/Appendix.tex}
\cleartoleftpage
\input{Backmatter/Backpage.tex}

\end{document}