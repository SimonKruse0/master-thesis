...
class Test1c(GeneralSetup):

    def objective_function(self,x):
        x = x/100
        result =  4*cos(2*x * (2 * pi))*x
        if result.ndim == 0:
            return result
        else:
            return result[0]
...
class Test3b(GeneralSetup):
    ...
    def plot_objectiv_function(self,x, nr=1):
        ...
        gap = (np.sign(x-0.5) + 1)
        intercept = 150
        result = 50 * gap +np.sin(100*x)*10 + intercept + (30-90*gap)*(nr%2)
        if x >= 0.5 and nr<2:
            return np.nan
        if x <= 0.5 and nr>=2:
            return np.nan
...
class SimonsTest2Probibalistic(Benchmark):
    ...
    def fun(self,x, *args): 
        x = x/200 + 0.5
        self.nfev += 1
        result = 50 * (np.sign(x-0.5) + 1)+np.sin(100*x)*10 + 100+30*np.random.randint(2)
        if result.ndim == 0:
            return result
        else:
            return result[0]
